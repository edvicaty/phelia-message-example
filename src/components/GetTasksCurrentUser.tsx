...
export function GetTasksCurrentUser({ useModal, useState }: PheliaMessageProps) {
  ...
  //Modal form function
  const openModal = useModal("modal", GetTasksCurrentUserModal, async (event) => {
    user = event.user
    form = event.form
    ...
    const fetchedTasks = await getFilteredTasks(String(currentUser.data().clickUpID))
